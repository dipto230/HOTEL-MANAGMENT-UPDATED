...
const AllRooms = () => {
  ...
  const { rooms = [], navigate, currency } = useAppContext() // default to empty array
    ...
    <div className='flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24 xl:px-32'>
      ...
      <div>
        ...
        {filteredRooms.map((room) => (
          <div
            key={room._id}
            className="bg-white rounded-xl shadow-md overflow-hidden mb-8 flex flex-col md:flex-row gap-6 hover:shadow-lg transition-shadow"
          >
            <img
              ...
              className='w-full md:w-1/2 h-64 object-cover cursor-pointer'
            />
            <div className="flex flex-col justify-between p-5 md:w-1/2">
              <div className="space-y-1">
                <p className="text-gray-500 text-sm">{room.hotel.city}</p>
                <h2 className="text-lg font-semibold text-gray-800">{room.hotel.name}</h2>
              </div>
      ...
      <div className="bg-white w-80 border border-gray-300 text-gray-600 max-lg:mb-8 lg:mt-16">
        <div className={`flex items-center justify-between px-5 py-2.5 min-lg:border-b border-gray-300 ${openFilters && "border-b"}`}>
          ...
          <div className='text-xs cursor-pointer lg:flex gap-2 items-center'>
